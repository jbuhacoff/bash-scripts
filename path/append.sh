...
# usage: append <path-var-name> <item> ...
# example: append PATH /path/to/scripts
...
# TODO: if the item is already in the list, remove it from the middle and append it
# so the postcondition can be the item is always at the end of the list
append_item() {
    local item="$1"
    if [ -z "$edited_path" ]; then
        ...
        if [ $? -ne 0 ]; then
            # TODO: if the item is already in the list, remove all instances before appending
            # append: new item at end of path
            edited_path="${edited_path}:${item}"
        fi    
    fi
}
